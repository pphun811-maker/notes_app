...
/// The desktop (Windows) interface.
///
/// This is the phase-2 UI, moved here unchanged so the phone can be re-skinned
/// without disturbing the PC. It is **frozen** for now: the user wants Android
/// finished first, and this is the interface they are already using every day.
///
/// The only edit made during the move was to call [formatNoteSubtitle] instead of
/// its own private date formatter, so the "do not zero-pad the date" decision has
/// a single home. The shown text is unchanged.
...
///
/// Kept on the desktop side even though Windows never needs it, so this file
/// remains a byte-for-byte copy of the phase-2 interface.
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            FilledButton.icon(
              ...
              label: const Text(NotesStrings.grantAccess),
            ),
            const SizedBox(height: 8),
            TextButton(
              onPressed: onGranted,
              child: const Text(NotesStrings.recheckAccess),
            ),
          ],
        ),
      ),
